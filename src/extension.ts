	...
	function getTerminalCommand(type?: string, name?: string, path?: string): string {
		...
		if (type && name && path) {
			if (type.toLowerCase() !== 'entity') {
				...
				command = generateEntity(name, path);
			}
		}
		return command;
	}
...
function generateEntity(name: string, path: string): string {
	let command = "(echo '// This class was generated by mijnCaress-generator";
	command += '\n';
	command += "export default class " + capitalize(name) + " {";
	command += '\n\n';
	command += "     constructor() {";
	command += '\n';
	command += "     }";
